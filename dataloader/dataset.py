...
class CLIP_COCO_dataset(Dataset):
    ...
    def __init__(self, config, text_tokenizer, context_length=77, input_resolution=224, clip_token=False, clip_prepro=None):
        ...
        self.clip_prepro = clip_prepro


    def tokenize(self, text):
        # may have errors
        if self.clip_token:
            result = clip.tokenize(text)
    ...
    def __getitem__(self, idx):
        ...
        if self.clip_prepro is None:
            img_input = self.transform(img)
        else:
            img_input = self.clip_prepro(img)
