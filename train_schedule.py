    ...
    for epoch in range(int(config.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % config.gradient_accumulation_steps == 0:
                ...
                # logit scaling set as max 100 as mentioned in CLIP paper # log(100) = 4.6052, minimum temperatue 0.01 and maximum temperature 1.0
                if config.n_gpu == 1:
                    model.logit_scale.data = torch.clamp(model.logit_scale.data, 0, 4.6052)
                    if config.increase_temp:
                        model.logit_scale.data = torch.clamp(model.logit_scale.data, 0, torch.log(logit_scale).data) # can not decrease temp
                elif config.n_gpu > 1:
                    model.module.logit_scale.data = torch.clamp(model.module.logit_scale.data, 0, 4.6052)
                    if config.increase_temp:
                        model.module.logit_scale.data = torch.clamp(model.module.logit_scale.data, 0, torch.log(logit_scale).data) # can not decrease temp
...
def main():
    ...
    parser.add_argument("--increase_temp", default=None, type=int, required=False)
    ...
    if args.increase_temp :
        config.increase_temp = args.increase_temp
